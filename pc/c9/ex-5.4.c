...
int compare_strings( char s1[], char s2[])
{
	...
	char null_zero[] = {"\0"};
	bool found_nedele = 0;
	bool found_nedele2 = 0;
	bool found_nedele3 = 0;
	...
	for( i = 0; i < original_string_length; ++i)
	{
		...
		// I cant seem to figure how rite this with out hard coding it and adiing insane numebrs 
	 // of flags

		for(j = i+j; j > 0; --j)
		{
			if(temp_haystack[2] == s2[2] )
			{
				found_nedele2 = 1;
				//printf("%s %s", temp_haystack, s2);
				//printf("yupii");
	...
	if(found_nedele && found_nedele2 && found_nedele3)
	{
		printf("yupii");
